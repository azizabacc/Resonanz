...
# peaks by eye
  dname='Aufg2_700mApeaks.dat'
  ncolsd=4

  data_array, info_dict =\
    readColumnData(dname, ncolsd, delimiter=' ', pr=False)

# print what we got:
  tp=data_array[:,0] # 1st column
  phip=data_array[:,1] # 2nd column
  td=data_array[:,2] # 3rd column
  phid=data_array[:,3] # 4th column
...
#           also Schätzungen der Parameterwerte
  ...
  p0p=[3., 2., 0.001] # initial parameters 
  envd=env_linexp_d
  p0d=[3., 2, -0.01]  
...
# plot data and analysis results
  ...
  plt.plot(t, phi)
  #plt.plot(tp, phip, 'rx', alpha=0.5, label='peak')
...
#  ax2.grid()
  ...
  #plt.savefig('Aufgabe2_700mAueb.pdf')

  #bestimme peridoendauer
  var=0.
  i=0
  #print tpn
  while i<5:
  ...
  print 'T', T
  #print var
  # Dämpfungsverhältnis k bestimmen
  # über 1. Summe
  n=5.
  i=0
  araya=np.zeros(n)
  ...
  while i<n:
      ...
      arayb[i]=phip[i+1]
      i=i+1
  ka=1/n*np.sum(araya/arayb)
  betaa=np.log(ka)/T
  
  # über 2.Wurzel
  #kb=np.sqrt(np.sqrt(np.sqrt(np.sqrt(np.sqrt(phip[0]/phip[int(n)])))))
  #print 'p0',phip[0]
  #print 'pn',phip[10]
  #kb=1.383202
  betab=np.log(kb)/T
